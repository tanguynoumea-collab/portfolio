...
---
*Last updated: 2026-05-28 after Phase 5 completion (Project Content Pipeline) — project case-study system live: 12 enriched bilingual MDX bodies (4-section case-study structure: Contexte/Défi/Processus/Résultat) with optional `gallery?` frontmatter ; `/[locale]/projects/[slug]` detail page (Server Component) via RELATIVE dynamic MDX import + `generateStaticParams` (12 static routes, `dynamicParams=false`, `next-mdx-remote` avoided — archived) + `notFound()` ; long-form magazine layout (cover hero → discriminated metadata strip → max-w-prose MDX → gated gallery grid → prev/next wrap) ; 3 custom MDX components (Image shadcn-Dialog zoom + data-lenis-prevent, CodeBlock `<pre>` override reusing rehype-pretty-code + clipboard copy, Callout info/warning/note palette-aliased) wired in `mdx-components.tsx` ; `useParallax` hook (matchMedia dual-branch, scrub:0.5, never re-registers ScrollTrigger) driving `ProjectCover` island (cover-only, reduced-motion gated). 276/276 Vitest tests green (222 Phase 4 + 54 net : +31 MDX components, +10 useParallax, +9 page/cover, +4 gallery-type). CONTENT-01/02/03 + ANIM-02 all validated automatically (20/20 must-have truths + `npm run build` exit 0 emitting 12 project routes) ; 4 manual browser checks tracked in 05-HUMAN-UAT.md. Added: `scripts/check-mdx-structure.ts` (CONTENT-01 gate), `projects.detail.*` i18n namespace (22 keys/locale, parity at 94 leaf paths), 24 placeholder gallery images. Zero new dependencies.*
